...
반드시 아래 JSON만 응답 (마크다운/코드블록 금지):
{
  ...
  "actionGuide": {
    "contact": "연락 — 오전/오후/저녁 중 언제가 좋은지 + 메시지 톤/방식 + 상대 반응에 따른 분기 로직(답장이 오면/늦으면 각각 대응) (3문장, 시계 시각 금지 — '오전', '오후', '저녁'으로만 표현)",
    "meeting": "만남 — 오전/오후/저녁 중 언제 + 어떤 분위기의 장소 + 선택→리스크 구조 포함 (2~3문장, 시계 시각 금지)",
    "conversation": "대화 — 주제 + 질문 vs 공유 비율 + 조건 문장(상대 반응이 미적지근하면/감정 표현이 있다면) (2~3문장)"
  },
  "timeFlow": {
    "morning": "오전 미션 — 오전에 할 구체적 행동 1가지 + 선택→리스크 포함 (2문장, '오전에'로 시작)",
    "afternoon": "오후 미션 — 오후에 할 구체적 행동 1가지 + 조건 문장 포함 (2문장, '오후에'로 시작)",
    "night": "저녁 미션 — 저녁에 할 구체적 행동 1가지 + 반응 분기 포함 (2문장, '저녁에'로 시작)"
  },
...
export async function GET(request: Request) {
  ...
  const cacheKey = `fortune-adult:v4:${date}:${birthKey}:${gender}:${status}`
