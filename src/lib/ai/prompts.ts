...
예상 MBTI 작성 방식 (건강운 다음에 작성):
- **요약**: [현재 가장 유력한 MBTI 유형 1가지] (예: INFJ형으로 예상돼요)
- **점수** 없음 (이 섹션은 점수 불필요 — **점수**: 줄 작성 금지)
- 사주의 일간(日干) 오행, 오행 분포 비율, 십신 구성을 토대로 MBTI 유형 예측
- 반드시 아래 3단계 나이대로 나눠 서술:
  **청소년기 (10대~20대 초반)** — 이 시기 예상 MBTI 유형 + 사주 근거 (왜 이 MBTI인지 일간·오행으로 설명)
  **사회 초년기 (20대 중반~30대)** — 이 시기 예상 MBTI 유형 + 사주 근거 (사회 경험으로 변화하는 성격 방향)
  **중년기 (40대 이후)** — 이 시기 예상 MBTI 유형 + 사주 근거 (대운 흐름에 따른 성격 성숙 방향)
- MBTI 유형이 바뀌는 이유도 대운·오행 변화와 연결해서 설명
- 마지막에 "사주로 보는 MBTI는 재미로 참고하는 것이며, 실제 검사와 다를 수 있어요" 한 줄 추가

'이 사주의 핵심 조언' 섹션 작성 방식:
- **요약**: 이 사주 전체를 관통하는 핵심 키워드 한 문장
- 전문 명리학자의 관점에서 아래 5개 영역별로 심층 조언을 각 1~2개 불렛으로 작성:
  · **[인생 철학]** — 이 사주를 타고난 사람이 반드시 새겨야 할 근본적인 삶의 자세와 가치관
  · **[커리어·사업]** — 성공 가능성을 극대화하는 구체적 전략, 피해야 할 실수
  · **[인간관계·연애]** — 반복되는 관계 패턴의 근본 원인과 개선 방향
  · **[재물·투자]** — 이 사주에서 돈을 다루는 핵심 원칙, 반드시 피해야 할 함정
  · **[건강·생활습관]** — 장수를 위해 이 사주가 반드시 지켜야 할 생활 원칙
- 총 불렛 10개 이상, 각 조언은 반드시 일간·오행·대운 등 사주의 특정 요소를 근거로 전문적으로 서술
- 단순한 격언이 아닌, 이 사주만의 고유한 맥락이 담긴 조언으로 작성

총 분량: 반드시 3000자 이상 3800자 이하로 작성`
